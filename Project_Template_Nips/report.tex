...
gate is designed to read. A design-space sensitivity check around
the four-branch backbone is reported in Appendix~\ref{si:minimal}.
...
channel mixer, and time mixer together give the model coverage over
heterogeneous inductive biases, which is what the calendar gate then
routes between. The dataset we study is moderate in size and
single-household, so we additionally probe how sensitive the headline
numbers are to dropping individual branches.

\paragraph{Per-branch ablation.}
A paired Wilcoxon signed-rank test over 3{,}829 short-task test
windows (Table~\ref{tab:ablation}) shows that on this single
dataset the time-mixer branch is the most load-bearing component:
removing it is the only edit whose effect on MAE cannot be
distinguished from zero. The other three branches each contribute
small differences. We read this as evidence that the four-branch
coverage is partially redundant on this household, not that the
backbone is unnecessary in general; on a more variable load profile
the redundant inductive biases would be expected to matter.
...
\paragraph{Stripped variant.}
A stripped configuration that retains only the persistence anchor,
the time-mixer branch, and the calendar gate (Minimal-A,
Figure~\ref{fig:minimal}) tracks the full TC-DPMixer on the weekly
task at $211.73 \pm 0.92$\,Wh/h with 116K parameters, which is
consistent with the calendar gate being the dominant component on
this dataset. Removing the gate from the same configuration
(Minimal-B) reverts the result to $214.83 \pm 0.50$\,Wh/h, the
full-model level. We keep the four-branch backbone as the headline
configuration because its inductive-bias coverage is what the gate
is designed to route between; the stripped variant is reported here
as a sensitivity check rather than as a replacement.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/fig_minimal.pdf}
  \caption{\textbf{Stripped-variant sensitivity check, 5 seeds, mean
    $\pm$ std.} Weekly task (left): Minimal-A keeps only the
    persistence anchor, the time-mixer branch, and the calendar gate;
    Minimal-B drops the gate from the same backbone. Short task
    (right): all three variants cluster within noise.}
  \label{fig:minimal}
\end{figure}
