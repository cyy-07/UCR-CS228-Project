...
while the trend weight rises as activity picks up. Around 2--3\,a.m.,
when household demand follows a regular nightly baseline, the gate
places almost all the weight on the seasonal branch. Around
5--7\,p.m., when usage becomes activity-dependent --- cooking,
returning home, evening appliances --- the gate shifts toward the
trend branch, which is responsive to the most recent observations.
Perturbing outdoor temperature by $\pm 10\,^{\circ}$C produces zero
RMS change in the gate output, since temperature is not part of its
input.
...
0.06\,Wh --- the model relies on the temporal signal the calendar
gate is designed to read. A stripped variant that retains only the
persistence anchor, the time-mixer branch, and the calendar gate
remains competitive (Appendix~\ref{si:minimal}), suggesting that
the calendar-conditioned routing captures a large fraction of the
weekly forecasting signal.
...
what matters is whether the next operating window will contain a
demand \emph{spike} that strains the grid. We therefore recast the
same input as a binary classification problem --- for each of the
next $H$ steps, predict a $\{0, 1\}$ label indicating whether the
load exceeds a training-set percentile threshold --- and replace
TC-DPMixer's regression head with a class-balanced BCE classification
head while keeping the four-branch backbone unchanged. Under the two
threshold choices we tested (top 10\,\%, $p_{90}$, and top 5\,\%,
$p_{95}$, of the training distribution), the backbone attains macro
AUC-ROC of 0.81 and 0.79 respectively --- the highest among all
contenders we ran, including a flat logistic-regression baseline at
0.79 / 0.77 (full per-horizon table in Appendix~\ref{si:spike}). On
an entirely held-out week the 168-hour forecast already tracks both
the diurnal baseline and the individual spikes hour-by-hour
...
The four-branch TC-DPMixer reaches the lowest weekly MAE in our
five-seed run while staying within 1\,Wh of iTransformer on the
short horizon at one-sixth the parameter count; its calendar gate
emits a non-trivial 24-hour routing schedule that is readable
directly off clock time. The horizon split mirrors this mechanism
--- the weekly task is dominated by calendar structure and
TC-DPMixer leads on every block-CV fold; the short task is dominated
by appliance inertia, on which the calendar gate carries no
information, and iTransformer's cross-channel attention takes the
0.84\,Wh advantage there. The same backbone, repurposed with a
binary head, also serves as a spike detector --- the operationally
useful generalization for a utility operator. The work is scoped to
one household over 4.5 months, and the weekly Wilcoxon
$p{=}0.0625$ is at the $n{=}5$ exact-test floor, so the bootstrap CI
is what we lean on. Cross-household transfer of the learned 24-hour
routing schedule is the question we did not answer here.
...
heterogeneous inductive biases, which is what the calendar gate then
routes between. To probe how sensitive the headline numbers are to
...
the time-mixer branch, and the calendar gate
...
the calendar gate is designed to route between; the stripped
...
\begin{figure}[h]
  ...
  \caption{\textbf{Stripped configuration sensitivity check, 5 seeds,
    ...
    time-mixer branch, and the calendar gate; the variant on the
