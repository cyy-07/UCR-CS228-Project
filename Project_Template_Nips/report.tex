...
averaged over 168\,h). Persistence has $R^{2}{=}-0.42$ on the short
split --- worse than a constant-mean predictor --- which is why every
learned model is built on the persistence residual
$\hat{y}=x_{t,0}+f(X)$.
...
\paragraph{Ablation and gate analysis.}
...
gate, since temperature is not part of its input. A paired Wilcoxon
test over the 3{,}829 short-task windows
(Table~\ref{tab:ablation}) localises the useful signal to the
time-mixer plus gate: removing the persistence anchor, decomposition,
or channel mixer each produces a small but significant MAE drop,
removing the time-mixer is non-significant, and keeping only
persistence is significantly worse.
...
\begin{table}[h]
  ...
  \caption{Branch ablation against full DPMixer (MAE 35.96 Wh,
    3{,}829 test windows). $\Delta$\,MAE is the change after removing
    the named component; significance is a paired Wilcoxon signed-rank
    test.}
  \label{tab:ablation}
  \begin{tabular}{lcc}
    \toprule
...
schedule is non-trivial, the paired-Wilcoxon ablation shows only the
time-mixer is irreplaceable, and a 116K minimal variant matches the
187K backbone at 38\,\% fewer parameters. The horizon split mirrors
this mechanism --- the weekly task is dominated by calendar
structure and TC-DPMixer leads on every block-CV fold; the short
task is dominated by appliance inertia and iTransformer is
0.84\,Wh ahead. The work is scoped to one household over 4.5 months,
