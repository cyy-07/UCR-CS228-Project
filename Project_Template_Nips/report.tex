...
gate, since temperature is not part of its input.
...
load exceeds the training-set $p_{95}$ threshold --- and replace
TC-DPMixer's regression head with a class-balanced BCE classification
head while keeping the four-branch backbone unchanged. The same
architecture transfers to this discriminative setting, and on an
entirely held-out week the 168-hour forecast already tracks both the
diurnal baseline and the individual spikes hour-by-hour
(Figure~\ref{fig:spike}). The result is that the inductive biases
built into the backbone for regression carry over to the operationally
